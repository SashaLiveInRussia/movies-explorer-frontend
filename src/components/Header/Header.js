		...
		<header>
			{auth === false && (
				<div className={'header'}>
					<Link className='header__logo' to='/'><img alt='логотип' src={logo} /></Link>
					<div className='header__auth'>
						<Link className='header__reg' to='/signup'>Регистрация</Link>
						<Link to='/signin' className='header__login-background'>
			...
			{auth === true && (
				<div className={"header" + (menu ? ' header__open' : '')}>
					<Link className="header__logo" to='/'><img alt='логотип' src={logo} /></Link>
					<button type='button' onClick={toggleMenu} className='header__burger'>
						<svg width="40" height="43" viewBox="0 0 40 43" fill="none" xmlns="http://www.w3.org/2000/svg">
							<path fillRule="evenodd" clipRule="evenodd" d="M31 15L9 15V12.5L31 12.5V15Z" fill="black" />
							<path fillRule="evenodd" clipRule="evenodd" d="M31 24L9 24V21.5L31 21.5V24Z" fill="black" />
							<path fillRule="evenodd" clipRule="evenodd" d="M31 33L9 33V30.5L31 30.5V33Z" fill="black" />
						</svg>
					</button>
					<div className="header__menu">
						...
						<NavLink to='/profile' className='header__account-background'>
							<span className='header__auth'>Аккаунт</span>
						</NavLink>
					</div>
				</div>
			)}
